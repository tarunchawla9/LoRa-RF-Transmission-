...
#define REG_PKT_SNR_VALUE 0x1b //0x19
...
#endif
char receiveLora = 0;
void enablefreqhopping() {
writeRegister(REG_HOP_PERIOD,
0x01);
}
int getFhssChannel() {
int channel = (readRegister
(REG_HOP_CHANNEL) & 0x3F);
return channel;
}
void enablenodeaddressing() {
writeRegister(REG_PACKET_CONFIG_1,
((readRegister
(REG_PACKET_CONFIG_1) &
(0xf9)) | 0x02));
//writeRegister(REG_PACKET_CONFIG_1, (readRegister(REG_PACKET_CONFIG_1)| (0x02)));
printf("%c\n", readRegister
(REG_PACKET_CONFIG_1));
}
void setnodeaddress() {
...
// set LNA boost
writeRegister(REG_LNA, readRegister
(REG_LNA) | 0x03);
...
// wait for TX done
while ((readRegister(REG_IRQ_FLAGS)
& IRQ_TX_DONE_MASK) == 0)
;
// clear IRQ’s
...
int packetLength = 0;
int irqFlags = readRegister
(REG_IRQ_FLAGS);


if (size > 0) {
implicitHeaderMode();
writeRegister(REG_PAYLOAD_LENGTH,
size & 0xff);
} else {
explicitHeaderMode();
}
...
float packetSnr() {
return ((int8_t) readRegister
(REG_PKT_SNR_VALUE)) * 0.25;
}
...
size_t size) {
int currentLength = readRegister
(REG_PAYLOAD_LENGTH);
size_t i = 0;
// check size
...
writeRegister(REG_PAYLOAD_LENGTH,
currentLength + size);
//checking len - this is unnecessary but the delay is useful in repeated sending
printf("payload len %d should be %d\n",
...
int currentAddress = readRegister
(REG_FIFO_ADDR_PTR);
// read
...
void receive(int size)
{
	...
	uint64_t frf = ((uint64_t)
	frequency << 19) / 32000000;
	writeRegister(REG_FRF_MSB,
	(uint8_t) (frf >> 16));
	writeRegister(REG_FRF_MID,
	(uint8_t) (frf >> 8));
	writeRegister(REG_FRF_LSB,
	(uint8_t) (frf >> 0));
	}
	...
	if (sf == 6) {
	writeRegister(
	REG_DETECTION_OPTIMIZE, 0xc5);
	writeRegister(
	REG_DETECTION_THRESHOLD, 0x0c);
	} else {
	writeRegister(
	REG_DETECTION_OPTIMIZE, 0xc3);
	writeRegister(
	...
	(readRegister(REG_MODEM_CONFIG_1) & 0x0f)
	| (bw << 4));
	}
	...
	writeRegister(REG_MODEM_CONFIG_1,
	(readRegister
	(REG_MODEM_CONFIG_1) & 0xf1)
	| (cr << 1));
	}
	void setPreambleLength(long length)
	{
	writeRegister(REG_PREAMBLE_MSB,
	(uint8_t) (length >> 8));
	writeRegister(REG_PREAMBLE_LSB,
	(uint8_t) (length >> 0));
	}
